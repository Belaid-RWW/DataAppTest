...
import kotlinx.android.synthetic.main.fragment_add_entreprise.*
import kotlinx.android.synthetic.main.fragment_add_entreprise.view.*
import kotlinx.android.synthetic.main.fragment_add_immeuble.view.*


class AddFragmentEntreprise : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view =  inflater.inflate(R.layout.fragment_add_entreprise, container, false)
        ...
        val dom : AutoCompleteTextView =  view.findViewById(R.id.addDom)
        ...
        val act : AutoCompleteTextView =  view.findViewById(R.id.addAct)
        ...
        val hor1 : AutoCompleteTextView =  view.findViewById(R.id.addHorDeb)
        ...
        val hor2 : AutoCompleteTextView =  view.findViewById(R.id.addHorFin)
    ...
    private fun insertDataToDatabase2() {
        ...
        val nomP = addNomPresta.text.toString()
        val dom = addDom.text.toString()
        val act = addAct.text.toString()
        val hor_deb = addHorDeb.text.toString()
        val hor_fin = addHorFin.text.toString()
        val tel = addTel.text
        val fax = addFax.text
        val gsm = addGsm.text
        val email = addEmail.text.toString()
        val site = addSite.text.toString()
        val idimm = addIDImm.text

        if (inputCheck(nomP, dom, act, hor_deb, hor_fin, tel, fax, gsm, email, site, idimm)){
            val entreprise = Entreprise(
                0, nomP, dom, act, hor_deb, hor_fin,
                Integer.parseInt(tel.toString()),
                Integer.parseInt(fax.toString()),
                Integer.parseInt(gsm.toString()), email, site, Integer.parseInt(idimm.toString())
            )

            mEntrepriseViewModel.addEntreprise(entreprise)
            Toast.makeText(requireContext(), "Entreprise ajoutée", Toast.LENGTH_LONG).show()
            findNavController().navigate(R.id.action_addFragmentEntreprise_to_listFragmentEntreprise)
        }
        else{
    ...
    private fun inputCheck(nompp: String, domm:String, actt:String, hor_debb:String, hor_finn: String, tell: Editable,
                           faxx : Editable, gsmm: Editable, emaill:String, sitee:String, idimm:Editable): Boolean{
        return !(TextUtils.isEmpty(nompp) && TextUtils.isEmpty(domm) && TextUtils.isEmpty(actt) && TextUtils.isEmpty(hor_debb)
                ...
                && TextUtils.isEmpty(sitee) && idimm.isEmpty())
